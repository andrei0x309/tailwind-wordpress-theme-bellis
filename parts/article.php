...
id="post-<?php the_ID();?>" data-id="<?php the_ID();?>" data-slug="<?php echo $post->post_name; ?>" data-title="<?php echo $post->post_title; ?>">
     <header>
        ...
        <div class="blog-post-meta flex flex-row">
            ...
            <i class="icon-user-solid-square"></i><span class="inline" <?php if (!$articleFull): ?>  itemprop="author"<?php endif;?>> <?php the_author();?></span></a>
            </address>
            <span class="px-2 pt-3 pb-6"><i class="icon-calendar"></i> 
            <time datetime="<?php echo get_the_date('Y-m-d'); ?>" title="<?php echo get_the_date(); ?>" <?php if (!$articleFull): ?> itemprop="datePublished" <?php endif;?>>
...
if ($postThumbUrl) {?>
    ...
    ?>
               <img <?php if (!$articleFull): ?> itemprop="image"<?php endif;?> class="m-auto wp-post-image" width="601" height="338" loading="lazy" src="<?php echo theme_resize_img_src($postThumbUrl, 601) ?>" <?php echo ($alt) ? 'alt="' . $alt . '"' : 'alt'; ?> >
                <?php
echo '</div>'; ?>
