...
<?php endif; ?> class="<?php echo $articleFull? '':'content-va-on ' ?> dark:bg-dark-bg dark:text-dark-text p-8 bg-white break-word post-body mb-4 <?php echo implode(' ',get_post_class()) ?>" id="post-<?php the_ID(); ?>" data-id="<?php the_ID(); ?>" data-slug="<?php echo $post->post_name ;?>" data-title="<?php echo $post->post_title ;?>">
     <header>
        ...
        <div class="blog-post-meta flex flex-row">
            <address class="author px-2 pt-3 pb-6"><a rel="author" title="Author's page" href="<?php echo site_url(); ?>/author/blackellis/"><i class="icon-user-solid-square"></i><span class="inline" <?php if (!$articleFull): ?>  itemprop="author"<?php endif; ?>> <?php the_author(); ?></span></a></address> 
 ...
 if($postThumbUrl) { ?>
           ...
           class="post-image post-image-left p-0 <?php echo $articleFull ? '': 'post-image-link'; ?>"><?php } ?>
               ...
               ?>
               <img <?php if (!$articleFull): ?> itemprop="image"<?php endif; ?> class="m-auto wp-post-image" width="500" height="281" loading="lazy" src="<?php echo theme_resize_img_src($postThumbUrl,500) ?>" <?php echo ($alt) ? 'alt="'.$alt.'"': 'alt';  ?> >
                <?php
                echo '</div>'; ?>
